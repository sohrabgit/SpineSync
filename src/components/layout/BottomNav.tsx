...
export function BottomNav({ active, onChange, badges }: { active: TabId; onChange: (t: TabId) => void; badges?: Partial<Record<TabId, number>> }) {
  return (
    <nav aria-label="Primary" className="fixed inset-x-0 bottom-0 z-40 mx-auto max-w-md border-t border-line bg-panel/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-md">
      <ul className="grid grid-cols-4">
        {TABS.map(({ id, label, icon: Icon, knob }) => {
            ...
            <li key={id}>
              ...
              >
                ...
                >
                  ...
                  {!!badge && (
                    <span className="absolute -top-1 -right-1.5 grid min-w-[18px] place-items-center rounded-full border-2 border-panel bg-brand px-1 text-[10px] leading-[14px] font-bold text-bg">
                      {badge}
                    </span>
                  )}
                </span>
                <span className={cn('text-[10px] font-bold tracking-[0.1em] uppercase transition-colors', selected ? 'text-ink' : 'text-dim')}>{label}</span>
              </button>
            </li>
          )
        })}
